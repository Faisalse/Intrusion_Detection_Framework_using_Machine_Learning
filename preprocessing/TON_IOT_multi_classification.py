...
def data_load_global_attacked(path, name, value = 0.1):
    ...
    data_num = corrupt_dataframe(data_num, value)
...
def corrupt_dataframe(df, attacked_ratio):
    
    corrupted_df = df.copy()
    
    # Flatten indices of all elements in the DataFrame
    total_elements = df.size
    n_attacked = int(total_elements * attacked_ratio)
    
    # Randomly select which positions to corrupt
    row_indices = np.random.randint(0, df.shape[0], size=n_attacked)
    col_indices = np.random.randint(0, df.shape[1], size=n_attacked)
    
    for r, c in zip(row_indices, col_indices):
        # Only corrupt numeric types to avoid errors
        try:
            val = corrupted_df.iat[r, c]
            if pd.api.types.is_numeric_dtype(type(val)):
                multiplier = attacked_ratio
                corrupted_df.iat[r, c] = val * multiplier
        except Exception:
            continue  # Skip if any issue arises (e.g., with non-numeric data)
